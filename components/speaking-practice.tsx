...
export function SpeakingPractice() {
  ...
  return <section className="mt-7 max-w-3xl rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-6 shadow-[0_18px_48px_rgba(35,44,43,0.05)] sm:p-8"><p className="text-xs font-bold tracking-[0.14em] text-[#d76a47]">{eyebrow}</p><h2 className="mt-2 font-serif text-4xl">{title}</h2><p className="mt-3 text-sm leading-6 text-[#69736e]">{description}</p><div className="mt-8 grid gap-4 sm:grid-cols-2"><PracticeCard icon="learning" title={learning} description={learningDescription} partOne={partOne} partTwo={partTwo} mode="learning" /><PracticeCard icon="test" title={test} description={testDescription} partOne={partOne} partTwo={partTwo} mode="test" /></div></section>;
}
