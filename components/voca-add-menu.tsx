...
import { BookOpenCheck, ClipboardPaste, FilePenLine, FolderOpen, Plus, Sparkles, X } from 'lucide-react';
...
type ManualSense = { partOfSpeech: 'n' | 'v' | 'a' | 'ad' | 'prep' | 'phrase' | 'conj'; text: string };

const blankSense = (): ManualSense => ({ partOfSpeech: 'n', text: '' });
...
export function VocaAddMenu() {
  ...
  const [manualSenses, setManualSenses] = useState<ManualSense[]>([blankSense()]);
  ...
  function updateManualSense(index: number, changes: Partial<ManualSense>) {
    setManualSenses((current) => current.map((sense, senseIndex) => senseIndex === index ? { ...sense, ...changes } : sense));
  }

  async function saveManualWord(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const senses = manualSenses.map((sense) => ({ ...sense, text: sense.text.trim() })).filter((sense) => sense.text);
    if (!manualWord.trim() || !manualPronunciation.trim() || !senses.length) {
      toast.add({ title: '영단어, 발음, 뜻을 입력해 주세요.', type: 'warning' });
      return;
    }
    ...
    try {
      const response = await fetch('/api/vocabulary', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'manual', word: manualWord, pronunciationIpa: manualPronunciation, senses }),
      });
      const result = await readResponse<{ created?: boolean; listId?: number; error?: string }>(response);
      if (!response.ok || !result.listId) throw new Error(result.error ?? '단어를 저장하지 못했습니다.');
      setManualOpen(false);
      setManualWord('');
      setManualPronunciation('');
      setManualSenses([blankSense()]);
      toast.add({
        title: result.created ? '단어를 추가했어요.' : '이미 추가한 단어예요.',
        description: '‘직접 추가한 단어’ 목록에서 확인할 수 있어요.',
  ...
  return (
    <div className="relative">
      ...
      {menuOpen && <><button type="button" aria-label="단어 목록 추가 메뉴 닫기" className="fixed inset-0 z-30" onClick={() => setMenuOpen(false)} /><div className="absolute right-0 top-[calc(100%+0.55rem)] z-40 w-56 overflow-hidden rounded-2xl border border-[#dcd6ca] bg-[#fffdf8] p-1.5 shadow-[0_18px_42px_rgba(29,41,53,0.16)]"><button type="button" onClick={() => { setMenuOpen(false); setLibraryOpen(true); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-[#35423e] transition-colors hover:bg-[#edf4ef]"><FolderOpen className="size-4 text-[#3f7a58]" aria-hidden="true" />저장된 단어 불러오기</button><button type="button" onClick={() => { setMenuOpen(false); toast.add({ title: 'AI 단어 생성은 다음에 합니다.', type: 'info', timeout: 1_000 }); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-[#35423e] transition-colors hover:bg-[#f8eee7]"><Sparkles className="size-4 text-[#d76a47]" aria-hidden="true" />AI로 새로 만들기</button><button type="button" onClick={() => { setMenuOpen(false); setManualOpen(true); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-[#35423e] transition-colors hover:bg-[#edf4ef]"><FilePenLine className="size-4 text-[#3f7a58]" aria-hidden="true" />단어 추가하기</button><button type="button" onClick={() => { setMenuOpen(false); toast.add({ title: '데이터 불러오기는 다음에 합니다.', description: 'Google 스프레드시트 연동으로 준비할 예정이에요.', type: 'info', timeout: 2_000 }); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-[#35423e] transition-colors hover:bg-[#f1eee7]"><ClipboardPaste className="size-4 text-[#7a7062]" aria-hidden="true" />데이터 불러오기</button></div></>}

      {manualOpen && <><button type="button" aria-label="단어 추가 창 닫기" className="fixed inset-0 z-40 bg-[#1d2935]/25 backdrop-blur-[1px]" onClick={closeManualForm} /><section role="dialog" aria-modal="true" aria-label="단어 추가하기" className="fixed inset-x-4 top-1/2 z-50 mx-auto max-h-[calc(100dvh-2rem)] w-auto max-w-lg -translate-y-1/2 overflow-y-auto rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-5 shadow-[0_24px_70px_rgba(29,41,53,0.2)] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-[0.14em] text-[#3f7a58]">MY VOCABULARY</p><h2 className="mt-1 font-serif text-2xl">단어 추가하기</h2><p className="mt-2 text-sm leading-6 text-[#707873]">입력한 단어는 ‘직접 추가한 단어’ 개인 목록에 저장됩니다.</p></div><button type="button" aria-label="닫기" onClick={closeManualForm} className="grid size-9 shrink-0 place-items-center rounded-lg text-[#68736e] hover:bg-[#f1ede5]"><X className="size-5" aria-hidden="true" /></button></div><form className="mt-6 space-y-5" onSubmit={saveManualWord}><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold text-[#52605b]">영단어<Input required value={manualWord} onChange={(event) => setManualWord(event.target.value)} placeholder="example" className="mt-2 h-11 border-[#d8d0c3] bg-[#fffefa]" /></label><label className="block text-sm font-semibold text-[#52605b]">발음<Input required value={manualPronunciation} onChange={(event) => setManualPronunciation(event.target.value)} placeholder="[ɪɡˈzæmpəl]" className="mt-2 h-11 border-[#d8d0c3] bg-[#fffefa]" /></label></div><div><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-[#52605b]">뜻</p><button type="button" onClick={() => setManualSenses((current) => [...current, blankSense()])} className="text-xs font-semibold text-[#3f7a58] hover:text-[#28533d]">+ 뜻 추가</button></div><div className="mt-2 space-y-2">{manualSenses.map((sense, index) => <div key={index} className="flex gap-2"><select aria-label={`${index + 1}번째 뜻 품사`} value={sense.partOfSpeech} onChange={(event) => updateManualSense(index, { partOfSpeech: event.target.value as ManualSense['partOfSpeech'] })} className="h-11 w-20 shrink-0 rounded-lg border border-[#d8d0c3] bg-[#fffefa] px-2 text-sm text-[#52605b] outline-none focus:border-[#3f7a58]"><option value="n">n.</option><option value="v">v.</option><option value="a">a.</option><option value="ad">ad.</option><option value="prep">prep.</option><option value="phrase">phr.</option><option value="conj">conj.</option></select><Input required value={sense.text} onChange={(event) => updateManualSense(index, { text: event.target.value })} placeholder="한국어 뜻" className="h-11 border-[#d8d0c3] bg-[#fffefa]" />{manualSenses.length > 1 && <button type="button" aria-label={`${index + 1}번째 뜻 삭제`} onClick={() => setManualSenses((current) => current.filter((_, senseIndex) => senseIndex !== index))} className="grid size-11 shrink-0 place-items-center rounded-lg text-[#8c5b4b] hover:bg-[#f9e8e1]"><X className="size-4" aria-hidden="true" /></button>}</div>)}</div></div><button type="submit" disabled={manualSubmitting} className="h-11 w-full rounded-xl bg-[#1d2935] text-sm font-semibold text-[#fffdf8] transition-transform hover:bg-[#344451] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60">{manualSubmitting ? '저장 중…' : '단어 추가하기'}</button></form></section></>}
