...
export function VocaAddMenu() {
  ...
  const byTitleDescending = (left: VocabularyList, right: VocabularyList) => right.title.localeCompare(left.title, 'en', { numeric: true });
  ...
  return (
    <div className="relative">
      ...
      {libraryOpen && <><button type="button" aria-label="저장된 단어 불러오기 닫기" className="fixed inset-0 z-40 bg-[#1d2935]/25 backdrop-blur-[1px]" onClick={() => setLibraryOpen(false)} /><section role="dialog" aria-modal="true" aria-label="저장된 단어 불러오기" className="fixed inset-x-4 top-1/2 z-50 mx-auto max-h-[calc(100dvh-2rem)] w-auto max-w-lg -translate-y-1/2 overflow-y-auto rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-5 shadow-[0_24px_70px_rgba(29,41,53,0.2)] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-[0.14em] text-[#d76a47]">VOCABULARY LIBRARY</p><h2 className="mt-1 font-serif text-2xl">저장된 단어 불러오기</h2><p className="mt-2 text-sm leading-6 text-[#707873]">내 개인 목록은 바로 열고, 공통 목록은 추가한 뒤 모든 기기에서 이어서 볼 수 있어요.</p></div><button type="button" aria-label="닫기" onClick={() => setLibraryOpen(false)} className="grid size-9 shrink-0 place-items-center rounded-lg text-[#68736e] hover:bg-[#f1ede5]"><X className="size-5" aria-hidden="true" /></button></div>
        {loading ? <p className="py-12 text-center text-sm text-[#77807a]">목록을 불러오는 중입니다.</p> : <div className="mt-6 space-y-5"><div><p className="mb-2 text-xs font-bold tracking-[0.12em] text-[#8a918b]">내가 추가한 목록</p>{savedLists.length ? <div className="max-h-[20.5rem] space-y-2 overflow-y-auto pr-1">{[...savedLists].sort(byTitleDescending).map((list) => <button key={list.id} type="button" onClick={() => openList(list)} className="flex w-full items-center gap-3 rounded-2xl border border-[#ded7ca] px-4 py-3 text-left hover:border-[#a8c9b2] hover:bg-[#f6fbf7]"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#e8f0eb] text-[#38634f]"><BookOpenCheck className="size-4" aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{list.title}</span><span className="mt-0.5 block text-xs text-[#77807a]">{list.wordCount}개 · {list.scope === 'common' ? '공통' : '개인'}</span></span></button>)}</div> : <p className="rounded-2xl bg-[#f5f2eb] px-4 py-3 text-sm text-[#78807a]">아직 추가한 단어 목록이 없습니다.</p>}</div><div><p className="mb-2 text-xs font-bold tracking-[0.12em] text-[#8a918b]">공통 단어 목록</p>{catalogLists.length ? <div className="max-h-[20.5rem] space-y-2 overflow-y-auto pr-1">{[...catalogLists].sort(byTitleDescending).map((list) => <div key={list.id} className="flex w-full items-center gap-3 rounded-2xl border border-[#ded7ca] px-4 py-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#f8eadf] text-[#c86442]"><FolderOpen className="size-4" aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{list.title}</span><span className="mt-0.5 block text-xs text-[#77807a]">{list.wordCount}개 · 공통</span></span><button type="button" onClick={() => addCommonList(list)} className="rounded-lg bg-[#1d2935] px-3 py-2 text-xs font-semibold text-white transition-transform hover:bg-[#344451] active:scale-95">추가</button></div>)}</div> : <p className="rounded-2xl bg-[#f5f2eb] px-4 py-3 text-sm text-[#78807a]">추가할 공통 목록이 없습니다.</p>}</div></div>}
      </section></>}
    </div>
  );
}
