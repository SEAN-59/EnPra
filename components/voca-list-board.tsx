...
function MaterialSymbol({ children, size = 'text-[18px]' }: { children: string; size?: 'text-[14px]' | 'text-[18px]' }) {
  return <span className={`material-symbols-rounded ${size}`} aria-hidden="true">{children}</span>;
}
...
function WordStatusButtons({ word, pending, onChange }: { word: VocabularyWord; pending: boolean; onChange: (changes: Partial<Pick<VocabularyWord, 'learningStatus' | 'isImportant'>>) => void }) {
  ...
  return <div className="flex shrink-0 items-center gap-1" aria-label={`${word.word} 학습 상태`}>
...
export function VocaListBoard() {
  ...
  async function removeList(list: VocabularyList) {
    ...
    try {
      const response = await fetch(`/api/vocabulary/list/${list.id}`, { method: 'DELETE' });
      const result = await readResponse<{ error?: string }>(response);
      if (!response.ok) throw new Error(result.error ?? '단어 목록을 지우지 못했습니다.');
      setLists((current) => current.filter((item) => item.id !== list.id));
      setListMenuId(null);
      toast.add({ title: list.scope === 'common' ? '단어 목록을 내 목록에서 제거했어요.' : '단어 목록을 지웠어요.', type: 'success' });
    } catch (error) {
  ...
  return (
    <section className="mt-7 w-full" aria-labelledby="saved-list-title">
      ...
      {loading ? <p className="rounded-2xl border border-[#ded7ca] bg-[#fffdf8] px-5 py-10 text-center text-sm text-[#77807a]">목록을 불러오는 중입니다.</p> : lists.length ? <div className="space-y-3">{lists.map((list) => <article key={list.id} className="flex items-center gap-2 rounded-2xl border border-[#ded7ca] bg-[#fffdf8] p-4 text-left shadow-[0_8px_20px_rgba(35,44,43,0.035)] transition-colors hover:border-[#e6b7a5] hover:bg-[#fff8f4]"><button type="button" onClick={() => void selectList(list)} className="flex min-w-0 flex-1 items-center gap-4 text-left"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#e8f0eb] text-[#38634f]"><BookOpenCheck className="size-5" aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="truncate font-semibold">{list.title}</span><span className="shrink-0 text-xs text-[#8a918b]">{list.scope === 'common' ? '공통' : '개인'}</span></span><span className="mt-1 block text-sm text-[#6e7772]">{list.wordCount}개</span></span></button><div className="relative shrink-0"><button type="button" aria-label={`${list.title} 목록 설정`} aria-expanded={listMenuId === list.id} onClick={() => setListMenuId((current) => current === list.id ? null : list.id)} className="grid size-9 place-items-center rounded-xl text-[#69736e] transition-colors hover:bg-[#f1ede5] active:scale-95"><Settings className="size-4" aria-hidden="true" /></button>{listMenuId === list.id && <><button type="button" aria-label="목록 설정 닫기" className="fixed inset-0 z-10 cursor-default" onClick={() => setListMenuId(null)} /><div className="absolute right-0 top-[calc(100%+0.35rem)] z-20 w-44 overflow-hidden rounded-xl border border-[#dcd6ca] bg-[#fffdf8] p-1.5 shadow-[0_14px_30px_rgba(29,41,53,0.14)]"><button type="button" onClick={() => { setListMenuId(null); toast.add({ title: '학습현황 초기화는 준비 중입니다.', type: 'info' }); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-[#4e5a55] hover:bg-[#f1ede5]"><MaterialSymbol size="text-[14px]">replay</MaterialSymbol>학습현황 초기화</button><button type="button" disabled={removingListId === list.id} onClick={() => void removeList(list)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-[#c3533e] hover:bg-[#fff0eb] disabled:opacity-50"><Trash2 className="size-3.5" aria-hidden="true" />목록 지우기</button></div></>}</div></article>)}</div> : <div className="rounded-3xl border border-dashed border-[#d8d0c3] bg-[#fbf9f4] px-6 py-12 text-center"><BookOpenCheck className="mx-auto size-6 text-[#9ba39d]" aria-hidden="true" /><p className="mt-4 font-semibold text-[#52605b]">아직 불러온 단어 목록이 없습니다.</p><p className="mt-2 text-sm leading-6 text-[#7a827d]">VOCA 상단의 + 버튼에서 공통 목록을 추가하거나, 개인 단어 목록을 만들어 보세요.</p></div>}
    </section>
  );
}
