    ...
    return (
      <section className="mt-7" aria-labelledby="word-list-title">
        ...
        <div className="overflow-hidden rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] shadow-[0_18px_48px_rgba(35,44,43,0.05)]">
          {selected.words.map((item, index) => <article key={item.word} className="grid gap-4 border-b border-[#e7e0d5] px-5 py-5 last:border-b-0 sm:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)] sm:items-center sm:gap-8 sm:px-7"><div className="flex items-baseline gap-3"><span className="w-5 shrink-0 text-xs font-semibold text-[#a3aaa5]">{index + 1}</span><h3 className="min-w-0 font-serif text-3xl font-semibold tracking-[-0.035em] text-[#293632] sm:text-4xl">{item.word}</h3></div><div className="border-l border-[#e9e1d6] pl-4 text-[15px] leading-7 text-[#59645e] sm:pl-7 sm:text-base">{item.meanings.map((meaning) => <p key={meaning}>{meaning}</p>)}</div></article>)}
        </div>
      </section>
    );
  }
