...
export function ManageCopyClient() {
  ...
  const selectedScreenEntries = useMemo(
    () => selectedScreen === null ? [] : entries.filter((entry) => entry.screenId === selectedScreen.id).sort((a, b) => a.variableName.localeCompare(b.variableName)),
  ...
  function openMockup(screen: Screen) {
    ...
    setSelectedMockupEntryId(entries.find((entry) => entry.screenId === screen.id)?.id ?? null);
...
function CopyMockupEditor({ screen, entries, selectedEntry, onSelectEntry, onChangeEntry, onSave, saving, canSave, texts }: { screen: Screen | null; entries: Entry[]; selectedEntry: Entry | null; onSelectEntry: (entryId: number) => void; onChangeEntry: (draftText: string) => void; onSave: () => void; saving: boolean; canSave: boolean; texts: { eyebrow: string; title: string; description: string; noScreen: string; editTitle: string; editEmpty: string; source: string; draft: string; save: string } }) {
  ...
  const bodyEntries = entries.filter((entry) => entry.id !== titleEntry?.id && entry.id !== descriptionEntry?.id);
  const isAction = (entry: Entry) => entry.draftText.length <= 18 && !/[.!?。]/.test(entry.draftText) && entry.textFormat === 'plain';
  const node = (entry: Entry, style = '') => <button type="button" key={entry.id} onClick={() => onSelectEntry(entry.id)} className={`group relative text-left outline-none transition ${selectedEntry?.id === entry.id ? 'ring-2 ring-[#d76a47] ring-offset-2' : 'hover:ring-2 hover:ring-[#e7b7a8] hover:ring-offset-2'} ${style}`}><span className="pointer-events-none absolute -top-2 left-2 z-10 rounded bg-[#26353b] px-1.5 py-0.5 text-[9px] font-semibold tracking-wide text-white opacity-0 transition group-hover:opacity-100">{entry.variableName}</span>{entry.draftText}</button>;
  return <section id="copy-mockup-editor" className="rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-5 sm:p-7"><div className="flex flex-col gap-2 border-b border-[#ece7dd] pb-5"><p className="text-xs font-bold tracking-[0.16em] text-[#d76a47]">{texts.eyebrow}</p><div className="flex flex-wrap items-baseline justify-between gap-3"><h2 className="font-serif text-2xl text-[#26353b]">{texts.title}</h2><code className="text-xs text-[#718079]">{screen.routePath ?? screen.screenKey}</code></div><p className="text-sm leading-6 text-[#69736e]">{texts.description}</p></div><div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]"><div className="overflow-hidden rounded-2xl border border-[#ddd6ca] bg-[#f6f2ea] p-3 sm:p-5"><div className="rounded-xl border border-[#e5dfd5] bg-[#fffdf8]"><div className="flex items-center justify-between border-b border-[#eee9e0] px-4 py-3"><div className="h-3 w-20 animate-pulse rounded-full bg-[#e9e4da]" /><div className="flex gap-2"><span className="h-2.5 w-8 animate-pulse rounded-full bg-[#eee9e0]" /><span className="h-2.5 w-8 animate-pulse rounded-full bg-[#eee9e0]" /></div></div><div className="grid min-h-[410px] grid-cols-[4.5rem_minmax(0,1fr)]"><aside className="border-r border-[#eee9e0] p-3"><div className="space-y-3">{Array.from({ length: 5 }, (_, index) => <span key={index} className="block h-2.5 animate-pulse rounded-full bg-[#eee9e0]" />)}</div></aside><div className="p-5 sm:p-7"><div className="mb-6 flex items-start justify-between gap-4"><div className="min-w-0">{titleEntry ? node(titleEntry, 'block font-serif text-3xl leading-tight text-[#26353b]') : <span className="block h-9 w-52 animate-pulse rounded bg-[#eee9e0]" />}{descriptionEntry ? node(descriptionEntry, 'mt-3 block max-w-2xl text-sm leading-6 text-[#69736e]') : <span className="mt-3 block h-3 w-80 max-w-full animate-pulse rounded bg-[#eee9e0]" />}</div><span className="h-9 w-20 shrink-0 animate-pulse rounded-xl bg-[#eee9e0]" /></div><div className="space-y-4">{bodyEntries.length ? bodyEntries.map((entry, index) => isAction(entry) ? node(entry, `inline-flex min-h-9 items-center rounded-xl px-3 text-sm font-semibold ${index % 3 === 0 ? 'bg-[#1d2935] text-white' : 'border border-[#d8d1c5] bg-white text-[#43514e]'}`) : <article key={entry.id} className="rounded-2xl border border-[#e5dfd5] bg-white p-4">{node(entry, 'block w-full whitespace-pre-wrap text-sm leading-6 text-[#43514e]')}</article>) : <div className="space-y-3"><span className="block h-20 animate-pulse rounded-2xl bg-[#eee9e0]" /><span className="block h-20 animate-pulse rounded-2xl bg-[#eee9e0]" /></div>}</div></div></div></div></div><aside className="rounded-2xl border border-[#e3ddd2] bg-[#faf7f0] p-4"><p className="text-xs font-bold tracking-[0.14em] text-[#d76a47]">{texts.editTitle}</p>{selectedEntry ? <div className="mt-4"><code className="text-xs font-semibold text-[#45665b]">{selectedEntry.variableName}</code>{selectedEntry.description && <p className="mt-1 text-xs leading-5 text-[#78817b]">{selectedEntry.description}</p>}<div className="mt-4"><span className="text-xs font-semibold text-[#69736e]">{texts.source}</span><p className="mt-1 rounded-xl border border-[#e4ddd2] bg-white px-3 py-2 text-xs leading-5 text-[#6b756f]">{selectedEntry.sourceText ?? selectedEntry.draftText}</p></div><label className="mt-4 block text-xs font-semibold text-[#69736e]"><span>{texts.draft}</span><textarea value={selectedEntry.draftText} onChange={(event) => onChangeEntry(event.target.value)} className="mt-1.5 min-h-36 w-full rounded-xl border border-[#d8d1c5] bg-white px-3 py-2.5 text-sm leading-6 text-[#344247] outline-none focus:border-[#cf7355] focus:ring-2 focus:ring-[#f3d7ca]" /></label><button type="button" onClick={onSave} disabled={!canSave || saving} className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#1d2935] px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45"><Save className="size-4" />{saving ? '저장 중' : texts.save}</button></div> : <p className="mt-4 text-sm leading-6 text-[#78817b]">{texts.editEmpty}</p>}</aside></div></section>;
}
