...
export function ManageCopyClient() {
  ...
  return <section className="mt-8 space-y-6">
    ...
    <section className="rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-5 sm:p-7"><SectionHead eyebrow="DRAFTS" title="문구 초안" action={<div className="flex flex-wrap justify-end gap-2"><button type="button" disabled={!screens.length} onClick={() => setEntryOpen((v) => !v)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#d9d2c6] px-3.5 text-sm font-semibold text-[#43514e] disabled:opacity-45"><FilePlus2 className="size-4" />새 문구</button>{entryEditing ? <><button type="button" onClick={() => { void load(false); setEntryEditing(false); }} className="inline-flex h-10 items-center gap-1.5 px-3 text-sm font-semibold text-[#66706d]"><X className="size-4" />취소</button><button type="button" onClick={() => void saveEntries()} disabled={savingEntries} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#1d2935] px-3.5 text-sm font-semibold text-white disabled:opacity-45"><Save className="size-4" />{savingEntries ? '저장 중' : '변경 저장'}</button></> : <button type="button" disabled={!entries.length} onClick={() => setEntryEditing(true)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#1d2935] px-3.5 text-sm font-semibold text-white disabled:opacity-45"><Pencil className="size-4" />수정</button>}</div>} />
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2 rounded-2xl border border-[#e7dfd2] bg-[#faf7f0] px-3 py-2.5 text-xs"><span className="font-semibold text-[#66716d]">현재 상태</span>{entryDirty.size > 0 && <DraftBadge tone="warning">저장 전 변경 {entryDirty.size}건</DraftBadge>}{unpublishedCount > 0 && <DraftBadge tone="pending">미발행 초안 {unpublishedCount}건</DraftBadge>}{entryDirty.size === 0 && unpublishedCount === 0 && <DraftBadge tone="published">{latest ? `발행본 v${latest.version} 반영됨` : '발행할 문구가 없습니다'}</DraftBadge>}<span className="text-[#858c85]">저장 전 → 미발행 → 반영 완료 순서로 관리돼요.</span></div>
      {!entries.length ? <div className="mt-5 rounded-2xl border border-dashed border-[#d8d0c2] px-5 py-12 text-center"><SlidersHorizontal className="mx-auto size-5 text-[#9ba39d]" /><p className="mt-3 font-medium text-[#43514e]">아직 등록된 문구가 없습니다.</p><p className="mt-1 text-sm text-[#77807a]">소속 화면을 선택하고 title 같은 변수명으로 문구를 등록하세요.</p></div> : <EntryTable entries={entries} screens={orderedScreens} editing={entryEditing} dirtyEntryIds={entryDirty} unpublishedEntryIds={unpublishedEntryIds} latestVersion={latest?.version ?? null} onChange={patchEntry} />}</section>

    <section className="rounded-3xl border border-[#dcd6ca] bg-[#fffdf8] p-5 sm:p-7"><div className="flex items-center justify-between"><span><span className="block text-xs font-bold tracking-[0.16em] text-[#d76a47]">RELEASE HISTORY</span><span className="mt-1 block font-serif text-2xl text-[#26353b]">발행본 이력</span></span><button type="button" onClick={() => setHistoryOpen((v) => !v)} aria-label={historyOpen ? '발행본 이력 닫기' : '발행본 이력 열기'} className="grid size-10 place-items-center rounded-xl border border-[#d9d2c6] text-[#566560] transition-colors hover:bg-[#f5f2ea]"><ChevronDown className={`size-5 transition-transform ${historyOpen ? 'rotate-180' : ''}`} /></button></div>{historyOpen && <div className="mt-5 space-y-3 border-t border-[#ece7dd] pt-5">{!publications.length ? <p className="text-sm text-[#77807a]">아직 만든 발행본이 없습니다.</p> : publications.map((item) => <article key={item.id} className="flex flex-col gap-3 rounded-2xl border border-[#e3ddd2] p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><strong className="font-serif text-xl">v{item.version}</strong><span className="rounded-full bg-[#e9f3ed] px-2 py-0.5 text-[11px] font-bold text-[#3f7158]">{item.status === 'published' ? '사이트 반영 완료' : item.status === 'ready' ? '발행본 생성됨' : item.status === 'failed' ? '생성 실패' : item.status}</span></div><p className="mt-1 text-xs text-[#747c76]">{item.copyCount}개 문구 · {dateText(item.publishedAt ?? item.createdAt)}{item.restoredFromPublicationId ? ` · v${item.restoredFromPublicationId}에서 복원` : ''}</p></div><button type="button" onClick={() => void publish(item.id)} disabled={publishing} className="inline-flex h-9 items-center gap-2 rounded-xl border border-[#d9d2c6] px-3 text-xs font-semibold text-[#43514e] disabled:opacity-45"><ArchiveRestore className="size-3.5" />이 버전으로 되돌리기</button></article>)}</div>}</section>
  </section>;
}
