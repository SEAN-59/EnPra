...
export function VocaAIGenerator() {
  ...
  async function generateVocabulary() {
    ...
    try {
      ...
      const result = await readResponse<{ listId?: number; addedCount?: number; partial?: boolean; attempts?: number; error?: string }>(response);
      if (!response.ok || !result.listId) throw new Error(result.error ?? 'AI 단어를 생성하지 못했습니다.');
      setProgress(100);
      window.setTimeout(() => {
